...
#define M1_ROWS_LENGTH 3600
#define M1_COLUMNS_LENGTH 3600
...
void* matrix_multiplier_openmp_metodo1(int rowsA, int colsA, int matrixA[rowsA][colsA], int rowsB, int colsB, int matrixB[rowsB][colsB]){
...
void calcula_matriz_openmp(int (*matriz_a)[M1_COLUMNS_LENGTH], int (*matriz_b)[M2_COLUMNS_LENGTH]){
    double begin = omp_get_wtime();
    int (*resultMatrix)[M2_COLUMNS_LENGTH] = matrix_multiplier_openmp_metodo1(M1_ROWS_LENGTH, M1_COLUMNS_LENGTH, matriz_a, M2_ROWS_LENGTH, M2_COLUMNS_LENGTH, matriz_b);
    double end = omp_get_wtime();
    double time_spent = end - begin;
